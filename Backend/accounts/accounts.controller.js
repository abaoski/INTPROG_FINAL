...
router.get('/test', (req, res) => {
    console.log('GET /accounts/test endpoint reached');
    res.json({ message: 'Accounts API is working' });
});
...
function register(req, res, next) {
    accountService.register(req.body, req.get('origin'))
        .then((result) => {
            // If this is the first user (admin)
            if (result && result.message && result.message.includes("Admin account created")) {
                res.json({
                    message: 'Admin account created successfully. You can now log in.',
                    account: result
                });
            }
            // If verification token is provided (fake backend mode)
            else if (result && result.verificationToken) {
            ...
            // For regular users in real backend mode
            else {
                res.json({ 
                    message: 'Registration successful, please check your email for verification instructions'
                });
