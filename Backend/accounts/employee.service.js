...
async function create(params) {
    ...
    await createWorkflowLog(employee.id, 'Added', {
        details: `${capitalizeFirstLetter(account.firstName)} ${capitalizeFirstLetter(account.lastName)} added${department ? ` to ${department.name}` : ''}`
    });
...
async function update(id, params) {
    ...
    if (params.departmentId && oldDepartmentId !== params.departmentId) {
        ...
        await createWorkflowLog(employee.id, 'Department Transfer', {
            details: `${capitalizeFirstLetter(account.firstName)} ${capitalizeFirstLetter(account.lastName)} transferred from ${oldDept ? oldDept.name : 'No Department'} to ${newDept.name}`
        });
    } else {
        // General update workflow
        await createWorkflowLog(employee.id, 'Updated', {
            details: `${capitalizeFirstLetter(account.firstName)} ${capitalizeFirstLetter(account.lastName)} information updated`
        });
    }
...
async function _delete(id) {
    ...
    await createWorkflowLog(employee.id, 'Deleted', {
        details: `${capitalizeFirstLetter(account.firstName)} ${capitalizeFirstLetter(account.lastName)} deleted`
    });
