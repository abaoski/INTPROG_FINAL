...
function createSchema(req, res, next) {
    const schema = Joi.object({
        ...
        status: Joi.string().required().valid('Approved', 'Rejected', 'Completed').default('Approved'),
...
function updateSchema(req, res, next) {
    const schema = Joi.object({
        ...
        details: Joi.string().allow('', null),
        status: Joi.string().valid('Approved', 'Rejected', 'Completed'),
        comments: Joi.string().allow('', null),
        handledBy: Joi.number().allow(null)
    });
    validateRequest(req, next, schema);
...
function updateStatusSchema(req, res, next) {
    const schema = Joi.object({
        status: Joi.string().required().valid('Approved', 'Rejected', 'Completed'),
        comments: Joi.string().allow('', null)
    });
    validateRequest(req, next, schema);
}
...
function updateStatus(req, res, next) {
    workflowService.updateStatus(req.params.id, req.body.status, req.body.comments)
        .then(workflow => res.json(workflow))
        .catch(next);
}
