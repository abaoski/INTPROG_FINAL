    ...
    private loadStoredAccount() {
        try {
            ...
            if (storedAccount) {
                ...
                // Validate the account has all required fields
                if (!account || !account.jwtToken) {
                    console.error('Invalid account data in localStorage');
                ...
                if (!this.isTokenValid(account)) {
                    ...
                    this.refreshToken().subscribe({
                        next: (refreshedAccount) => {
                            console.log('Token refreshed successfully on page load');
                        },
                        error: (error) => {
                            console.warn('Silent token refresh failed on page load:', error);
                            // Don't logout immediately in dev mode
                            if (!environment.production) {
                                console.warn('Development mode: Keeping invalid session active');
                            }
                        }
                    });
                } else {
            ...
            // Don't remove storage in dev mode to help with debugging
            if (environment.production) {
                localStorage.removeItem('account');
            }
        }
    ...
    refreshToken() {
        ...
        return this.http.post<any>(`${baseUrl}/refresh-token`, {}, { 
            ...
            catchError(error => {
                ...
                // Only clear account data if not in development mode
                if (environment.production) {
                this.clearAccountData();
                ...
                this.refreshTokenTimeout = setTimeout(() => {
                    ...
                    this.refreshToken().subscribe({
                        error: (error) => {
                            ...
                            // Only clear account data in production
                            if (environment.production) {
                            this.clearAccountData();
                            }
                        }
