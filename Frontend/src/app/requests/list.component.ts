        ...
        this.requestService.changeStatus(id, newStatus, `Status changed to ${newStatus} by admin`)
            ...
            .subscribe({
                next: (response) => {
                    ...
                    // If the response contains a workflow
                    if (response['workflows'] && response['workflows'].length > 0) {
                        request.workflows = response['workflows'];
                    }

                    request.isUpdating = false;
                    this.alertService.success(`Request status updated to "${newStatus}" successfully`);
                    
                    // Reload the requests after a short delay to ensure all related data is updated
                    setTimeout(() => this.loadRequests(), 500);
                },
                error: error => {
                    this.alertService.error(`Error updating status: ${error}`);
                    request.isUpdating = false;
                }
            });
    }
